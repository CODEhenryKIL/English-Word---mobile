...
### **[v1.3.0] 모바일 UI 사용자 경험(UX) 최적화**
- **학습 모드 개선**:
    - 카드 높이가 변해도 네비게이션 버튼이 화면 중앙에 고정되도록 수정 (Fixed Position).
    - 카드 내부 터치 영역과 간섭을 줄이고 모바일 그립감을 고려한 레이아웃 적용.
- **테스트 모드 개선**:
    - 문제 박스를 화면 상단으로 배치하여 모바일 키보드 활성화 시 가려짐 현상 해결.
- **배포 안정성 강화**:
    - Vercel 배포 시 환경 변수(`NEXT_PUBLIC_SUPABASE_URL` 등) 누락 감지 및 진단 페이지 구현.
    - 배포 자동화 스크립트(`deploy-helper.js`) 추가로 안전한 배포 프로세스 구축.
